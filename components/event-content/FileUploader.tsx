  ...
  useEffect(() => {
    ...
    _uppy.on('file-added', (file) => {
      ...
      _uppy.getPlugin('XHRUpload')?.setOptions({
        endpoint: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/${bucketName}/${objectName}`,
      })
    ...
    if (useTUS) {
      if (!uppyInstance.getPlugin('Tus')) {
        uppyInstance.use(Tus, {
          endpoint: `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/upload/resumable/${bucketName}`,
          limit: 6,
          headers: {
